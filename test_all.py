...
from utils import compare_floats_with_epsilon, sqrt, custom_pow, compare_with_list_of_floats
...
class TestSolver:
    ...
    def check_solution(coefs: Dict[float, float], expected_result: Tuple[Any, ...]):
        equation_solver: Solver = Solver(coefs, False)
...
class TestUtils:
    ...
    def test_custom_pow(self):
        ...
        for num, degree, expected_result in test_sets:
            assert compare_floats_with_epsilon(custom_pow(num, degree), expected_result)
        for num, degree in value_error_values:
            with pytest.raises(ValueError):
                print(num, degree)
                custom_pow(num, degree)
