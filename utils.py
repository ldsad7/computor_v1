...
def custom_pow(num: float, degree: float, epsilon: float = 10e-12) -> float:
    """
    I improved the code from https://stackoverflow.com/a/3519308/8990391
    """
    sign = 1
    ...
    if compare_floats_with_epsilon(degree, 0.0):
        return 1
    if degree < 0:
        return 1 / custom_pow(num, -degree, epsilon)
    if degree >= 10:
        return sign * sqr(custom_pow(num, degree / 2, epsilon / 2))
    if degree >= 1:
        return sign * num * custom_pow(num, degree - 1, epsilon)
    if epsilon >= 1:
        return sign * sqrt(num)
    return sign * sqrt(custom_pow(num, degree * 2, epsilon * 2))
